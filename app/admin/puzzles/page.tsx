...
export default function AdminPuzzlesPage() {
  ...
  const [editFolderOrder, setEditFolderOrder] = useState<number>(0);
  ...
  const handleUpdateFolder = async (e: React.FormEvent, folderId: string) => {
    e.preventDefault();
              ...
              {folders.map((folder) => {
                ...
                if (isEditing) {
                  return (
                    <form
                      key={folder.id}
                      onSubmit={(e) => handleUpdateFolder(e, folder.id)}
                      onClick={(e) => e.stopPropagation()}
                      className="bg-slate-900 p-6 rounded-2xl border border-blue-500/50 space-y-4 shadow-md"
                    >
                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 mb-1">Folder Name</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 mb-1">Order (Sequence Number)</label>
                        <input
                          type="number"
                          value={editFolderOrder}
                          onChange={(e) => setEditFolderOrder(parseInt(e.target.value, 10) || 0)}
                          className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-white font-bold outline-none focus:border-blue-500"
                        />
                      </div>
                      <div className="flex items-center gap-2 pt-2">
                        <button
                          type="submit"
                          className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-[11px] rounded-lg transition-colors"
                        >
                          Save
                        </button>
                        <button
                          ...
                          className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-extrabold text-[11px] rounded-lg transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={(e) => {
                          ...
                          setEditFolderOrder(folder.order || 0);
                        }}
                        className="p-2 text-slate-600 hover:text-blue-400 hover:bg-blue-950/40 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                        title="Edit Folder"
                      >
